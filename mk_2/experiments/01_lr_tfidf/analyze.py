"""
mk_2/experiments/01_lr_tfidf/analyze.py

Load the sweep results, generate the comparison table and the
C-vs-metric scatter plots.

Inputs:
    results/sweep.json     per-config metrics
    results/winners.json   best config per regime

Outputs:
    figures/winners_table.txt        text comparison table
    figures/C_vs_<metric>.png         one scatter per metric
"""
...
def main():
    ...
    print()
    print("=== Sweep results: TF-IDF + LR winners under three regimes ===")
    print()
    print(table)
    print()
    ...
    if spread < 2.0:
        ...
        print("  ==> regimes diverge strongly: each picks a meaningfully different C")

    # Cross-mk_1 comparison reminder
    print()
    print("--- cross-layer comparison hint ---")
    f1_f1      = winners["f1_tuned"]["f1_macro"]
    f1_maxent  = winners["maxent_tuned"]["f1_macro"]
    print(f"mk_2 (TF-IDF + LR) F1-tuned     : F1 = {f1_f1:.4f}")
    print(f"mk_2 (TF-IDF + LR) MaxEnt-tuned : F1 = {f1_maxent:.4f}")
    print(f"mk_1 (NB)          F1-tuned     : F1 = 0.8812  (for reference)")
    print(f"mk_1 (NB)          MaxEnt-tuned : F1 = 0.8493  (for reference)")
