"""
mk_3/experiments/01b_glove_lr/sweep.py

Random-search hyperparameter sweep over GloVe + LR.

Sampled space:
    C            : log-uniform on [1e-2, 1e2]
    pooling      : {'mean', 'max', 'tfidf-weighted-mean'}
    normalize    : {True, False}     (L2-normalize doc vectors after pooling)
    class_weight : {None, 'balanced'}

Embedding dim is fixed at 100 to keep the search tight (300d would be a
separate ablation).

GloVe table is loaded ONCE before the sweep and cached on each pooler
instance, so per-config fits don't re-read the 200 MB file.
"""
...
def sample_configs(n_iter: int, seed: int = 42) -> list[dict]:
    ...
    for _ in range(n_iter):
        configs.append({
            "C":            float(C_dist.rvs()),
            "pooling":      pooling_opts[int(rng.integers(0, 3))],
            "normalize":    bool(rng.choice([True, False])),
...
def build_pipeline(cfg: dict, glove_table: dict) -> Pipeline:
    """Build pipeline using a pre-loaded GloVe table."""
    pooler = GlovePooler(
    ...
    # inject the cached table so pooler.fit() doesn't re-read the file
    pooler.embeddings_ = glove_table
...
def main(n_iter: int = 30, seed: int = 42, beta: float = 0.5):
    ...
    print(">>> loading GloVe table (one-time, ~10-30s) ...", flush=True)
    t0 = time.time()
    ...
    for i, cfg in enumerate(configs, 1):
        ...
        norm_short = "T" if cfg["normalize"] else "F"
        pool_short = {"mean": "mean", "max": "max ", "tfidf-weighted-mean": "tfwm"}[cfg["pooling"]]
        print(
    ...
    for regime, rec in winners.items():
        ...
        print(
            f"  {regime:14s}  C={rec['C']:.4g}  pool={rec['pooling']}  "
            f"norm={rec['normalize']}  cw={cw_short}  "
